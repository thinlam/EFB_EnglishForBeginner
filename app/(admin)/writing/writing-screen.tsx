...
function wordsRangeLabel(min?: number, max?: number) {
  if (min == null && max == null) return '—';
  if (min != null && max != null) return `${min}–${max} từ`;
  if (min != null && max == null) return `${min}+ từ`;
  return `${max} từ`;
}
...
export default function WritingScreen() {
  ...
  const [items, setItems] = useState<WritingLesson[]>([]);
    ...
    try {
      ...
      const data = snap.docs.map((d) => {
        ...
        return {
          ...
          writingTips: raw.writingTips ?? '',
          imageUrl: raw.imageUrl ?? '',
          level: (raw.level as CEFR) ?? 'A1',
          topic: (raw.topic as Topic) ?? 'Daily Life',
          type: (raw.type as WritingType) ?? 'paragraph',
          bandMin: raw.bandMin ?? undefined,
          bandMax: raw.bandMax ?? undefined,
          wordMin: raw.wordMin ?? undefined,
          wordMax: raw.wordMax ?? undefined,
          createdAt:
      ...
      Alert.alert(
        'Lỗi tải dữ liệu',
        e?.message || 'Không lấy được danh sách Writing'
      );
    } finally {
      setLoading(false);
    }
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((it) => {
      const byText =
        ...
        it.writingTips?.toLowerCase().includes(t) ||
  ...
  const onEdit = (id: string) => {
    // giống Speaking: dùng chung màn create để edit
    router.push({
      pathname: '/(admin)/writing/writing-create',
      params: { id },
    });
  };
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Sửa', onPress: () => onEdit(id) },
        {
          ...
          onPress: async () => {
            try {
              await deleteDoc(doc(db, 'writing_lessons', id));
  ...
  const openScript = (item: WritingLesson) => {
    const prompt = item.prompt?.trim();
    const sample = item.sampleAnswer?.trim();
    const tips = item.writingTips?.trim();

    if (!prompt && !sample && !tips) {
      Alert.alert('Bài này chưa có nội dung.');
      return;
    }
    ...
    Alert.alert(item.title, text.trim());
  };

  // ============== RENDER ITEM ==============
  const renderItem = ({ item }: { item: WritingLesson }) => {
      ...
      <View style={S.card}>
        ...
        <View style={S.cardHeader}>
          <View style={{ flex: 1 }}>
            <View style={S.rowLine}>
              ...
              />
              <Text style={S.rowLabel}>Bài Writing:</Text>
            ...
            <View style={[S.rowLine, { marginTop: 4 }]}>
              ...
              {(item.wordMin != null || item.wordMax != null) && (
                <>
                  <Text style={[S.rowLabel, { marginLeft: 8 }]}>Số từ:</Text>
                  <Text style={S.rowText}>
                    {wordsRangeLabel(item.wordMin, item.wordMax)}
                  </Text>
                </>
              )}
            </View>
        ...
        <View style={[S.cardActions, { marginTop: 14 }]}>
          ...
          >
            ...
            <Text style={S.iconBtnText}>Xem đề & sample</Text>
          </TouchableOpacity>
  ...
  return (
    <View style={[S.container, { paddingTop: insets.top }]}>
      ...
      <View style={S.header}>
        ...
        <Text style={S.headerTitle}>Quản lý Writing</Text>
      ...
      <View style={[S.filterRow, { gap: 8 }]}>
        <View style={[S.searchBox, { flex: 1 }]}>
          ...
          />
          <TextInput
            placeholder="Tìm theo tiêu đề, prompt, sample…"
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={S.emptyWrap}>
              <Text style={S.emptyTitle}>Chưa có bài Writing</Text>
      ...
      <TouchableOpacity
        onPress={() =>
          router.push('/(admin)/writing/writing-create')
        }
